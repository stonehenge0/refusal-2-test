...
def substring_matching_judge_fn(
    ...
    if (
        completion is None
    ):  # Emma: Changed here cause of bugs, but am thinking to just leave this out completely...
        return False
...
def evaluate_jailbreak(
    ...
    """
    print(
        f"---Arguments for evaluate_jailbreak: : methods {methodologies}, eval_path {evaluation_path}, completions_path{completions_path}, completions{completions}"
    )
    ...
    if "wildguard" in methodologies:
        ...
        evaluation["wildguard_harmful"] = np.nanmean(
            [completion["wildguard"]["response_harmful"] for completion in completions]
        )
        evaluation["wildguard_refusal"] = np.nanmean(
            [completion["wildguard"]["refusal"] for completion in completions]
        )
